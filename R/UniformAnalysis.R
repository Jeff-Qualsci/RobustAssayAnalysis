...
UniformData <- read_csv('Data/AllData.csv') |>
  ...
  nest() |>
  mutate(ExpTime = mdy_hms(ExpTime), ExpDate = as_date(ExpTime)) |>
  unnest(cols = c(data)) |>
  ungroup() |>
  group_by(PlateId) |>
  ...
  mutate(
    Scale = if_else(Scale == 'Data', 'Raw', 'Median'),
...
T1Raw <- ggplot(
  filter(UniformData, Assay == 'Tgt1', Scale == 'Raw'),
  aes(x = PlateId, y = Data, color = PlateType)
) +
  geom_boxplot() +
  labs(x = 'Plate', y = 'Raw Data') +
...
T2Raw <- ggplot(
  filter(UniformData, Assay == 'Tgt2', Scale == 'Raw'),
  aes(x = PlateId, y = Data, color = PlateType)
) +
  geom_boxplot() +
  labs(x = 'Plate', y = 'Data') +
...
T1Med <- ggplot(
  filter(UniformData, Assay == 'Tgt1', Scale == 'Median'),
  aes(x = PlateId, y = Data, color = PlateType)
) +
  geom_boxplot() +
  labs(x = 'Plate', y = 'Median Scaled Data') +
  scale_color_viridis_d() +
  theme_classic() +
  theme(axis.text.x = element_blank()) +
  theme(legend.position = 'none') +
...
T2Med <- ggplot(
  filter(UniformData, Assay == 'Tgt2', Scale == 'Median'),
  aes(x = PlateId, y = Data, color = PlateType)
) +
  geom_boxplot() +
  labs(x = 'Plate', y = 'Median Scaled Data') +
