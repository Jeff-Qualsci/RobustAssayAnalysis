...
for (i in 1:length(InputFiles)) {
  TimeStamp <- read_tsv(
    file = paste0('Plates/', InputFiles[i]),
    skip = 1548,
    col_names = c('ExpTime'),
    n_max = 1
  )
...
TempData <- TempData |>
  ...
  mutate(
    PlateType = if_else(
      str_detect(File, '3727'),
      'A',
      if_else(str_detect(File, '782'), 'B', 'C')
    ),
    Assay = if_else(
      str_detect(File, 'dhe') |
        str_detect(ExpTime, '1/14') |
        str_detect(ExpTime, '12/10'),
      'Tgt1',
      'Tgt2'
    ),
